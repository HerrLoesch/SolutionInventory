        ...
        >
          <div class="d-flex align-center" style="gap: 4px">
            ...
            <v-btn
              size="x-small"
              variant="text"
              @click="options.includedStatuses = RING_META.map((r) => r.label.toLowerCase())"
              >All</v-btn
            >
            <v-btn size="x-small" variant="text" @click="options.includedStatuses = []">None</v-btn>
          </div>
          <div v-for="ring in RING_META" :key="ring.label" style="display: flex; align-items: center; gap: 4px">
            <v-checkbox
            ...
            <span
              :style="{
                ...
                height: '14px',
                borderRadius: '50%',
                background: options.statusColors[ring.label.toLowerCase()],
                flexShrink: 0,
            ...
            <input
              :ref="
                (el) => {
                  if (el) colorInputs[ring.label.toLowerCase()] = el
                }
              "
              type="color"
          ...
          <div class="d-flex align-center" style="gap: 4px">
            ...
            <v-btn size="x-small" variant="text" @click="options.categoryGroups.forEach((g) => (g.included = true))"
              >All</v-btn
            >
            <v-btn size="x-small" variant="text" @click="options.categoryGroups.forEach((g) => (g.included = false))"
              >None</v-btn
            >
          </div>
          <div
            ...
            :style="{
              borderRadius: '6px',
              border:
                catDragOver === group.key && catDragKey !== group.key ? '1px dashed #1565c0' : '1px solid transparent',
              background:
                catDragOver === group.key && catDragKey !== group.key ? 'rgba(21,101,192,.06)' : 'transparent',
              padding: '2px 4px',
              marginBottom: '2px',
              cursor: 'grab'
            }"
            ...
            @dragend="resetCategoryDrag"
          >
            <div style="display: flex; align-items: center; gap: 4px">
              <v-checkbox v-model="group.included" density="compact" hide-details style="flex-shrink: 0" />
              <v-text-field
                v-model="group.label"
                density="compact"
                variant="plain"
                ...
                style="flex: 1; min-width: 0"
              />
              <v-icon size="14" style="color: rgba(0, 0, 0, 0.3); flex-shrink: 0; cursor: grab">mdi-drag</v-icon>
            </div>
            <div
              v-if="group.categories.length > 1"
              style="display: flex; flex-wrap: wrap; gap: 4px; padding-left: 40px; margin-top: 2px; margin-bottom: 4px"
            >
              <v-chip
